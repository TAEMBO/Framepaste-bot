...
module.exports = {
    ...
    execute: async (client, member) => {
        ...
        const embed = new Discord.MessageEmbed()
            ...
            {name: '🔹 Join Date', value: `<t:${Math.round(new Date(member.joinedTimestamp) / 1000)}> - <t:${Math.round(new Date(member.joinedTimestamp - message.createdTimestamp) / 1000)}:R>`})
            .addField('🔹 Roles', `${member.roles.cache.size > 1 ? member.roles.cache.filter(x => x.id !== member.guild.roles.everyone.id).sort((a, b) => b.position - a.position).map(x => x).join(member.roles.cache.size > 4 ? ' ' : '\n').slice(0, 1024) : 'None'}`)
