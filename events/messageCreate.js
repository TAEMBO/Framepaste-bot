...
module.exports = {
	...
	if(message.channel.id === client.config.mainServer.channels.suggestions && message.author.id !== client.user.id && !client.hasModPerms(client, message.member)){ return message.delete(); }
	...
	if (message.mentions.roles.some(mentionedRole => punishableRoleMentions.includes(mentionedRole.id))) {
		...
		message.channel.awaitMessages({ filter, max: 1, time: 60000, errors: ["time"]}).then(async collected => {
			...
			try {
				const muteResult = await client.punishments.addPunishment("mute", message.member, { time: "5m", reason: "Useless staff ping", interaction: message}, collected.first().author.id);
			} catch (error) {
