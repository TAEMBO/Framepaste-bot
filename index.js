...
Object.assign(client.userLevels, {
	...
	_milestone() {
		const milestones = [10, 100, 1000, 69696, 100000, 420000, 696969, 800000, 1000000]; // always keep the previously achived milestone in the array so the progress is correct. here you can stack as many future milestones as youd like
	...
	if (message.content.startsWith(client.prefix)) {
		...
		if (commandFile) {
			...
			if (client.channelRestrictions._content[message.channel.id]?.includes(commandFile.category) || client.channelRestrictions._content[message.channel.id]?.some(x => x.includes(commandFile.name))) {
				if (!client.hasModPerms(client, message.member) && !message.member.roles.cache.has(client.config.mainServer.roles.levels.one.id))
				return (message.delete() && message.reply('Command is restricted in this channel, use <#902524214718902332>').then(m => setTimeout(() => m.delete(), 7000)))
			}
