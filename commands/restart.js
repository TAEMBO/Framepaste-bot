...
module.exports = {
  run: (client, interaction) => {
    ...
    client.channels.cache.get(client.config.mainServer.channels.modlogs).send({embeds: [new client.embed().setTitle(`**Restart issued by ${interaction.user.tag}**`).setColor(client.config.embedColor).setTimestamp()]}).then(async ()=> eval(process.exit(-1)))
  },
