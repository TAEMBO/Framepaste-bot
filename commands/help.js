...
function helpPage(pageNumber, client, message, args, toEdit = false) {
	...
	if (toEdit) {
		...
		message.reply({embeds: [embed], allowedMentions: { repliedUser: false }, components: [new e.MessageActionRow().addComponents(new e.MessageButton().setStyle("SECONDARY").setCustomId("back").setEmoji("⬅️"), new e.MessageButton().setStyle("SECONDARY").setCustomId("forward").setEmoji("➡️"))]})
...
module.exports = {
    run: (client, message, args) => {
		...
		if (parseInt(args[1])) {
			if (!client.commands.pages[parseInt(args[1]) - 1]) return message.reply({content: 'That page number doesn\'t exist.', allowedMentions: { repliedUser: false }});
