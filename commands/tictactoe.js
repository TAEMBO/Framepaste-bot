...
module.exports = {
	run: async (client, interaction) => {
		...
		if (interaction.options.getUser("member")) {
			request = `Does ${interaction.options.getUser("member").toString()} want to play Tic Tac Toe with ${interaction.member.toString()}? Respond with y/n to decide if you want to be elected Opponent. (60s)`;
			challenge = true;
		}
		interaction.reply({content: request}).then(a => {
			client.games.set(interaction.channel.id, interaction.user.tag);
			// wait until someone wants to be the opponent
			const filter = x => {
				if (challenge) {
					return ['y', 'n'].some(y => x.content.toLowerCase().startsWith(y)) && x.author.id === interaction.options.getUser("member").id;
				} else {
					return x.content.toLowerCase().startsWith('me');
				}
			}
			interaction.channel.awaitMessages({ filter, max: 1, time: 60000, errors: ['time']}).then(async b => {
				if (challenge) {
				...
				if (!opponent) return interaction.channel.send('Something went wrong! You have no opponent.');
				...
				const game = {
					...
					userError: (index) => {
						...
						interaction.channel.send(`${game.participants[game.turn].toString()} (\`${game.markers[game.turn]}\`) ${fouls[index]} ${consequence}`);
					...
					victoryAction: () => {
						game.ended = true;
						interaction.channel.send(`${game.boardState()}\n${game.participants[game.turn].toString()} (\`${game.markers[game.turn]}\`) Won the game!${game.singleplayer ? ' Singleplayer games are not counted in Tic Tac Toe Statistics.' : ' TIP: You can view Tic Tac Toe statistics with /ttt leaderboard`'}`);
				...
				await interaction.channel.send(`The origin point of the board is in the bottom left (0,0). The top right is (2,2). Syntax for placing your marker is \`[X position],[Y position]\`. 3 fouls and you're out. You can type \`/end\` to surrender on your own turn or \`/draw\` to suggest a draw to your opponent.\n${game.participants[0].toString()} is \`${game.markers[0]}\`\n${game.participants[1].toString()} is \`${game.markers[1]}\`\n\`${game.markers[0]}\` starts!`);
				...
				const cycle = () => { return new Promise(async (res, rej) => {
					...
					const result = await interaction.channel.send(game.boardState() + `\n${game.participants[game.turn].toString()}, Where do you want to place your \`${game.markers[game.turn]}\`? (60s)`).then(async c => {
						...
						return await interaction.channel.awaitMessages({ filter: filter2, max: 1, time: 60000, errors: ['time'] }).then(async e => {
							// ,end
							if (e.first()?.content.startsWith('end')) {
								await interaction.channel.send(`${game.participants[game.turn].toString()} (\`${game.markers[game.turn]}\`) wants to surrender!`);
								game.changeTurn();
								game.victoryAction();
								return res();
							// ,draw
							} else if (e.first()?.content.startsWith('draw')) {
								await interaction.channel.send(`${game.participants[game.nextTurn].toString()} (\`${game.markers[game.nextTurn]}\`), do you want to end the game in a draw? Respond with y/n. (60s)`);
								const fil = x => x.author.id === game.participants[game.nextTurn].user.id && ['y', 'n'].some(y => x.content.toLowerCase().startsWith(y));
								const opponentResponses = await interaction.channel.awaitMessages({ fil, max: 1, time: 60000, errors: ['time']}).catch(() => interaction.channel.send('Game does not end.'));
