		...
		if (args[1]) {
			if (args[1] === 'categorynames') {
				...
				const embed = new client.embed()
					...
					.addField(':small_blue_diamond: How come _you_ can use restricted commands?', `Anyone with the <@&${client.config.mainServer.roles.levels.one.id}> role can bypass channel restrictions.`)
					.setColor(client.embedColor)
				return message.channel.send(embed);
		 	} else {
