	...
	run: async (client, message, args) => {
		if (args[1]) {
			if (args[1].toLowerCase() === 'help') {
				...
				if (!endPart) return message.reply('You need to add a component.');
				if (endPart.length > 256) return message.reply('The component or name is too long.');
				const component = endPart.slice(0, colonIndex).trim();
				if (!component) return message.reply('You need to add a colon.');
				const name = endPart.slice(colonIndex + 1).trim();
				if (!name) return message.reply('You need to add a name.');
				...
				return message.reply(`Successfully added "${component}: ${name}" to your specs.`);
			} else if (args[1].toLowerCase() === 'edit') {
				if (!client.specsDb.hasUser(message.author.id)) return message.reply('You haven\'t added any specs.');
				const endPart = args.slice(2).join(' ');
				const colonIndex = endPart.indexOf(':') >= 0 ? endPart.indexOf(':') : endPart.indexOf(' ');
				if (!endPart) return message.reply('You need to add a component.');
				if (endPart.length > 256) return message.reply('The component or name is too long.');
				...
				const name = endPart.slice(colonIndex + 1).trim();
				if (!name) return message.reply('You need to add a name.');
				if (!client.specsDb.hasSpec(message.author.id, component)) return message.reply('You haven\'t added that spec.');
				client.specsDb.editSpecs(message.author.id, component, name);
				return message.reply(`Successfully edited "${component}", new value is "${name}".`);
			} else if (args[1].toLowerCase() === 'delete') {
				if (!client.specsDb.hasUser(message.author.id)) return message.reply('You haven\'t added any specs.');
				if (args[2]) {
					const component = args.slice(2).join(' ')
					if (!client.specsDb.hasSpec(message.author.id, args[2])) return message.reply('You haven\'t added that spec.');
					client.specsDb.deleteSpec(message.author.id, args[2]);
					return message.reply(`Successfully deleted "${args[2]}" from your specs.`);
				} else {
					client.specsDb.deleteData(message.author.id);
					return message.reply(`Successfully deleted all your specs.`);
				}
			} else {
				...
				if (!client.specsDb.hasUser(member.user.id)) return message.reply('They haven\'t added any specs yet.');
			...
			if (!client.specsDb.hasUser(message.author.id)) return message.reply(`You haven\'t added any specs yet. Do \`${client.prefix}specifications help\` to learn more.`);
