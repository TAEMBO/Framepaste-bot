...
module.exports = {
	run: async (client, interaction) => {
		...
		if (!client.config.eval.whitelist.includes(interaction.user.id) && !interaction.member.roles.cache.has(client.config.mainServer.roles.botdeveloper)) return interaction.reply({content: `You need the <@&${client.config.mainServer.roles.botdeveloper}> role to use this command.`, allowedMentions: {roles: false}});
